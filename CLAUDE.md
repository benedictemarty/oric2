...
| ADR-28 | Threading WM | Option C : politique fenêtre + rendu en tâche serveur WM, curseur seul en IRQ ; réf Intuition/GEOS. **§8 BASCULE PAR DÉFAUT (2026-06-10)** : le boot crée task_wm et pose `WM_TASKMODE=$A5` lui-même (panic R5 si échec) — le record des display-lists ADR-34 s'exécute hors IRQ ; opt-out explicite `TC_WM_LEGACY=$A5` (`--wm-legacy`, chemin legacy conservé et gardé par test rouge-checké) ; fix single-writer `$D0..$D9` (Forbid/Permit ADR-25 sur la section pop→push de task_wm). **Bascule VALIDÉE interactivement 2026-06-10 (drag fluide) — ADR-28 entièrement livrée** |
