...
### ADR-32 — Course ZP/registres IRQ↔tâche : invariant P1/P2/P3 (ratifiée 2026-06-10)
Le dossier instruisait 3 options (A sei épars / B migration mouse_step hors IRQ / C partition ZP) pour fermer la course ZP IRQ↔tâche. La mesure (§10.9) a montré que le bug déclencheur (clock/Opt-A) n'était PAS une collision ZP mais la **frame IRQ 8-bit vs contexte M=16** : `kernel_irq_handler` faisait `sep #$30` avant pha/phx/phy → A.high (registre B) du contexte interrompu remplacé par la valeur laissée par le code IRQ (site mesuré : `kernel_print_char`, C=$BC98→$5C98 au RTI). **Décision ratifiée = invariant à 3 protocoles** : **P1** enveloppe IRQ — le bloc souris save/restore les scratch ZP $08-$93 (`IRQ_ZP_SAVE` $019100), tous les syscalls protégés (remplace les sei de point Opt-A, retirés) ; **P2** sections critiques `php/sei…plp` côté tâche pour les structures RMW partagées (rings EVENT/RAW/KBD — pushers tâche `push_menu`/`push_verbatim` corrigés) ; **P3** frame IRQ 16-bit (`rep #$30`), forgeurs au format 10 octets [Y16][X16][A16][P][PCL][PCH][PBR]. Collatéraux fermés : TICK_COUNTER relocalisé $5500→$019093 (écrasait l'opcode `rti` du segment NMI_HANDLER) ; course exempt↔focus actée option (γ) telle qu'implémentée (`kernel_kbd_waiter_eligible` : slot unique + filtre focus/exempt). **4 gardes rouge→vert dans `make tests`** : `irq-frame-m16` (P3), `position-shift` v2.2 (P1), `evt-push-atomic` (P2), `nmi-safe`. Méthode : §10.3bis « mesurer, jamais deviner » — 4 faux candidats ZP écartés par mesure avant la cause réelle. **Non ratifié** : migration mouse_step hors IRQ (option B originelle, reste chantier ADR-28 gated `WM_TASKMODE`), layout ZP $E0-$EF (abandonné, P1 le rend inutile), coalesce-on-overflow RAW_RING. Invariant repris dans `OricOS/CLAUDE.md` §5nonies (+ checklist item 10). Cf. `docs/adr/0032-zp-race-irq-task.md` §10-§11.

### ADR-34 — GPU-ISA : primitives GUI, async, display-lists (étapes B et C ratifiées 2026-06-10)
Fixe le **contrat des primitives graphiques** pour que (1) la charge IRQ du rendu disparaisse et (2) un changement de carte/HDL ne change jamais les primitives. Références instruites : Apple IIgs (QuickDraw II = contrat logiciel, leçon de perf sans blitter), SymbOS (mêmes primitives CPC/V9938 command engine), Amiga (Blitter/Copper). Reco senior « B puis C », ratifiées séparément sur validation interactive. **Règles de contrat GRAVÉES** : sémantique d'opcode immuable, extensions additives uniquement, `GPU_CAPS` obligatoire (read TRIGGER = caps<<4|version, $F4 = v4), **Phosphoric = suite de conformance de l'ISA** (annexe DAT). **Étape B (v2, async)** : FIFO 16 + durée modélisée + IRQ de complétion (`IRQF_GPU`) + QFULL/OVF sticky ; kernel routé par capacités (`GPU_CAPS_KERNEL` boot, carte v1 → sync intact) ; compose post-and-continue −52 % (22 534→10 591 cyc). **Étape C (v3/v4, display-lists)** : `EXEC_LIST` ($09 — 13 o/entrée, terminateur $FF, récursion interdite, borne 64) + `EXEC_LIST_XY` ($0A — replay **translaté** de (dx, dy), ARG2 = dy<<12|dx 12-bit two's complement, translation en espace entier signé → clip négatif correct ; CLEAR/BLIT non translatés). Fenêtres (chrome + widgets), menu (liste 9), taskbar (liste 10, horloge directe) = display-lists rejouables, double-bufferées par slot (`WL_LISTS` SDRAM $030000) ; chaînes dans une **arène per-(slot, flip)** (`WL_ARENA` $038000, bump par record) ; garde 64 entrées + arène pleine → record avorté → fallback rendu direct. Drag = replay translaté (origine `WL_ORG` au record, move n'invalide plus) + **coalescing de frames** (geste + GPU BUSY → skip + rattrapage fin de geste) + **culling dirty-rect** (union ancien∪nouveau ; fenêtres/menu/taskbar intacts skippés). Fix collatéral : collision latente `WL_VALID`/`PANIC_CODE`/`BUNDLE_FOUND_*` ($019095 → bloc relogé $0191A0, asserts ld65). **Mesure : `redraw_drag` 13 151 → 1 307 cyc (−90,1 %)**, budget ≤ 2 200 gravé (R8). Chantier connexe hors ADR : record hors IRQ (converge ADR-28/`WM_TASKMODE`). Cf. `docs/adr/0034-gpu-isa-gui-primitives-DRAFT.md`.

---

*Extrait automatique — révisé 2026-06-10 (ratification ADR-34 B+C). Conserver en cohérence avec `CLAUDE.md` §2.*
