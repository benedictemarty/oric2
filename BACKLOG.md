...
| 🐞 **PH-test-winflaky** | **`test_oricos_win_draw` flaky (sensible au layout) — bloque les éditions `wm.s`** | Instruit 2026-05-27. **EXP1** : insérer 2 `nop` *inertes* après ~`wm.s:2296` (`_wm_scroll_update`, NON appelée par win_draw) fait **échouer** `win_draw`. **EXP2** : sonder `$10A032` à *chaque* pas (au lieu de tous les `0x7FFF`) échoue aussi → ce n'est pas une sonde grossière ratant un transitoire : avec le décalage, compose n'écrit **jamais** `$10A032=$FF`, alors que le backing store EST rempli (`$080000=$FF`) et la position fenêtre correcte (X=100,Y=80). Ce n'est ni `.smart`, ni la table syscall (symbolique/relogée). **Cause** : le test valide un **état composité transitoire** d'une tâche qui *crée→remplit→compose→`SYS_EXIT` (ferme la fenêtre)* sous scheduler préemptif ; le pixel attendu dépend d'interactions **timing/scheduling/Z-order** qui bougent au moindre changement de taille de code → faux-échecs sur éditions anodines (c'est ce qui a « cassé » mon fix scroll : pas le compositeur, juste le timing). **Fix** : rendre le test déterministe — la tâche win_draw ne sort PAS (boucle/yield après compose, fenêtre persistante), OU asserter sur backing-store + WM_TABLE (déterministe) plutôt que sur le pixel framebuffer transitoire. **Pré-req à toute édition de `wm.s` après ~l.2296** (donc au fix scroll WG_RELH). | petit | — |
...
| ~~**clock**~~ | ✅ **clos 2026-05-26** : première vraie app C pilotée par le temps (`apps/clock/`). Fenêtre + barre de progression rythmée par `SYS_GET_TICKS` ($1D, nouveau primitif = compteur ticks 8-bit) + `SYS_YIELD`, dessin `SYS_GFX_FILL_RECT`/`FLUSH`. SDK `oricos_get_ticks` (clobbers a/x/y — le COP ne préserve pas X/Y). `test_oricos_clock`. Refactor préalable : bundles d'apps → segment `BUNDLES` ($7000+). 584 verts. | — | TC-poc-hello-c ✅ |
...
| ID | Titre | Notes |
|----|-------|-------|
| ⚙️ **ADR-27** | **Modèle de backing store fenêtre — option (b) retenue** | Audit GPU 2026-05-27, Finding B (backing store 1 banque/slot + stride 512 → 128 lignes max). **Option (b) retenue** : stride GPU configurable + backing store compact. **✅ Référence GPU implémentée** (Phosphoric 1.22.88) : `GPU_OP_SET_BPL` + `gpu->bpl` (défaut 512), BLIT double-stride, helper `kernel_gfx_set_bpl` (ZP $90/$91), 2 tests, 594 verts. Contrainte tranchée : aucun port I/O libre → opcode (pas de registre dédié). **Reste (migration kernel, < 50 %)** : backing store compact/contigu + `bpl=byte_w` dans `kernel_wm_compose` + reset `bpl=0` avant `kernel_wm_redraw` + clip surface. Ratification ADR à finaliser au seuil 50 % (moratoire §10). DRAFT `docs/adr/0027-backing-store-fenetre-DRAFT.md`. |
...
| ⚙️ **SP-3.o — Contrôles valeur + ascenseurs + GenView (widgets GeoWorks)** (arc, cadré 2026-05-26 ; pré-req SP-3.n ✅) | **Objectif** : compléter la famille de **contrôles « valeur/saisie »** qui manque vs GeoWorks (cf. comparatif widget/fenêtrage : windowing déjà à parité, widgets pauvres). Aujourd'hui OricOS n'a que le cliquable (bouton/menu/dialogue) ; manquent **ascenseurs, sliders, champ texte, listes, radios** + l'**API de valeur de contrôle**. **Réf. d'art** : GeoWorks `GenScrollbar`/`GenView`/`GenBoolean`/`GenItemGroup`/`GenText`/`GenList` ; Control Manager IIgs (scroll bar, LineEdit, List Manager). **Cohérent ADR-26** (mêmes principes : contrôles déclarés en GenUI, interactions → `MSG_CONTROL` via MainLoop, rendu QDF). **Découpage** : **S.1** **API valeur de contrôle** : `SYS_CTL_GET_VALUE`/`SYS_CTL_SET_VALUE` (slots ABI réservés `$1B`+) + champ `value`/`max` dans le record widget (16B → étendre ou réutiliser couleur/cb) ; **implémente la checkbox** (`WG_TYPE_CHECK`, `GenBoolean`) : clic → toggle value → `MSG_CONTROL`+id, l'app lit la valeur. **S.2** **Ascenseurs** (`WG_TYPE_SCROLL_V/_H`, `GenScrollbar`) : rendu gouttière+thumb (+flèches), hit-test (thumb/gouttière/flèche), **thumb-drag dans le MainLoop** (état gardé entre appels, comme le drag de fenêtre → 1 événement/appel, pas de boucle longue sous Forbid) → `MSG_CONTROL`+nouvelle valeur. **S.3** **GenView** (le gros confort) : zone de contenu **scrollable managée** reliant fenêtre + 1-2 scrollbars + viewport ; v1 = l'app dessine son contenu, le système gère les scrollbars et rend la position ; clip du contenu à la charge de l'app. **S.4** autres contrôles valeur : radios (`GenItemGroup`, exclusion mutuelle), **champ texte éditable** (`GenText`/LineEdit : curseur, insertion, backspace via clavier au focus), liste (`GenList`). **S.5** tags déclaratifs GenUI : `GU_CHECK`/`GU_SCROLL_V`/`GU_SCROLL_H`/`GU_RADIO`/`GU_TEXT`/`GU_LIST` (chaînes inline comme GU_BUTTON). **S.6** démo C : fenêtre avec ascenseur + checkbox + champ texte, l'app lit les valeurs. **Risques** : thumb-drag (réutilise le pattern drag fenêtre, balisé) ; **GenView** = le plus lourd (coordination viewport/scroll/contenu, clip) ; champ texte (gestion curseur/édition, interaction clavier-au-focus G.3) ; record widget 16B peut-être trop court pour value+max+state (étendre WIDGET_ENTSZ → churn WIDGET_TABLE) ; churn tests WM/widgets. **Effort** ~3-4 sem (S.1+S.2 valeur+scrollbar ; S.3 GenView ; S.4 texte/liste/radio ; S.5+S.6 déclaratif+démo). **Ordre d'impact** : S.1 (débloque tout) → S.2 (ascenseurs, demandé) → S.3 (GenView) → S.4. **Avancement 2026-05-26** : **S.1 ✅** (`SYS_CTL_GET/SET_VALUE` $1B/$1C + `WG_TYPE_CHECK`, `kernel_ctl_toggle`, 575 verts). **S.2 ✅** (ascenseurs `WG_TYPE_SCROLL_V/_H` value(+14)/max(+15) ; thumb-drag MainLoop `SCROLL_DRAG_ID` + `_wm_scroll_update` value=clamp(souris-gouttière,0,max) ; rendu `kernel_tk_scrollbar` ; fix conflit drag fenêtre↔contrôle dans `wm_step_arm_drag` ; `test_oricos_scrollbar`, 576 verts). **S.3 (a+b) ✅** (`WG_TYPE_VIEW` viewport+scrollbar intégré `kernel_tk_view` ; drag→`scroll_y` réutilise `SCROLL_DRAG_ID` ; `test_oricos_genview`, 577 verts). **S.3c ✅** (tag déclaratif `GU_VIEW` dans `sys_ui_define`→GenView dans table GenUI `sud_view` ; app C `view_demo` déclare fenêtre+`GU_VIEW`, lit `scroll_y` via `SYS_CTL_GET_VALUE`/`oricos_ctl_get_value`+`oricos_msg_id` sur `MSG_CONTROL` ; `test_oricos_view_demo`, 578 verts. **Note** : flux scroll_y→C prouvé ; le redessin pixel clippé du contenu reste un suivi — tension surface widgets XVGA `$100000` vs backing-store app G.4). **S.4a ✅** (radios `WG_TYPE_RADIO` selected(+14)/group(+15) ; `kernel_ctl_radio_select` exclusion par group id ; dispatch clic MainLoop+desktop IRQ ; rendu case colorée ; `task_radio`, `test_oricos_radio`, 579 verts). **S.4b ✅** (champ texte éditable `WG_TYPE_TEXT` : buffer bank1 `TEXT_BUFS`+id*16 length(+14)/maxlen(+15) ; focus `TEXT_FOCUS_ID` au clic ; édition `_wm_text_edit` insertion/backspace pilotée par `mlc_key` ; rendu `kernel_tk_text_field` face+cadre+texte+curseur ; `task_text`, `test_oricos_text_field`, 580 verts). **S.4c ✅** (liste `WG_TYPE_LIST` : blob d'items slots 8 o, selected(+14)/count(+15) ; `kernel_tk_list` surlignage sélection ; `kernel_ctl_list_select` row=(souris-abs_y)/16 ; `task_list`, `test_oricos_list`, 581 verts. **Fix layout** : le CODE OricOS dépassait `$5000` et écrasait SENTINEL/VERSION → relocalisés `$016300`/`$016310`, plafond CODE = `$015400`). **S.4 ✅ complet** (radios + champ texte + liste). **S.5 ✅** (tags GenUI déclaratifs `GU_CHECK`/`GU_SCROLL_V`/`GU_SCROLL_H`/`GU_RADIO`/`GU_TEXT` dans `sys_ui_define` ; helpers `_sud_rect`/`_sud_attach` ; `task_genui`, `test_oricos_genui`, 582 verts ; **`GU_LIST` différé** : pointeur blob = bank app, mécanisme de copie à concevoir). **S.6 ✅** (capstone : app C `ctl_demo` déclare fenêtre + checkbox + ascenseur + champ texte via GenUI, lit la valeur du contrôle touché via `SYS_CTL_GET_VALUE` sur `MSG_CONTROL` ; `test_oricos_ctl_demo`, 583 verts). **Refactor layout** : bundles d'apps `.incbin` sortis du segment CODE vers segment `BUNDLES` ($7000+) → CODE redescend à ~$4327 (~3,8 Ko de marge regagnée sous le plafond $5400). **ARC SP-3.o CLOS.** **Polish post-arc** : **S.7** (redraw ciblé `kernel_wm_redraw_widget` — refactor `_wm_draw_widget_body` — repeint un seul contrôle, fin du scintillement plein écran au scroll/édition, 584 verts) ; **S.7 v2** (fix 2 bugs P0 critique senior : ascenseur bloqué à mi-course → `_wm_scroll_update` clampe `value` à la course de la gouttière `dim − SCROLL_THUMB_SZ` ; curseur disparu après redraw ciblé → `_wm_redraw_ctl` redessine le curseur après le repaint widget, 3 sites, 588 verts) ; **S.7 v2b** (fix régression : le bloc « course » 16-bit de v2 — rep/sep + immédiats — corrompait le rendu via mauvais tracking de mode ca65 et effaçait le corps des fenêtres au clic d'un contrôle ; réécrit en 8-bit pur ; vérifié par dump XVGA `$100000` ; 588 verts). Reste seulement, en option : `GU_LIST` déclaratif (pointeur blob = bank app, mécanisme de copie à concevoir) — parqué. **ADR** : extension mineure d'ADR-26 (nouveaux types de contrôle, même modèle) ; mention CHANGELOG, pas de nouvelle ADR sauf si GenView introduit un modèle de viewport non-trivial (à réévaluer en S.3). | SP-3.n ✅, ADR-26 ✅ |
