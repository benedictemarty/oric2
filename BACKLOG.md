...
| ID | Titre | Notes |
|----|-------|-------|
| 🔓 **ADR-27** | **Modèle de backing store fenêtre (ouverte — à instruire)** | Audit GPU 2026-05-27, Finding B. Backing store = 1 banque 64 KiB/slot + stride GPU figée 512 → **128 lignes max** ; `kernel_wm_compose` sur-lit les banques voisines pour fenêtre > 128 px (exposé par fix BLIT v0.2). DRAFT `docs/adr/0027-backing-store-fenetre-DRAFT.md` : options (a) multi-banques contigu, (b) stride GPU configurable [recommandée, aligne ADR-21 v0.2], (c) clamp 128 [non-viable], (d) dessin direct [hors-scope]. **NE PAS trancher** (moratoire §10). Mitigation v1 : apps ≤ 128 px en compose. |
