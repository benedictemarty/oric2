...
| **SP-VRAM-1** | Phosphoric : `src/io/vram_device.{c,h}` simulant 32 MiB SDRAM via I/O `$0330-$033C`. Tests unitaires + intégration. | ADR-19 ✅ |
| **SP-VRAM-2** | OricOS kernel : helpers `kernel_vram_read_block`, `kernel_vram_write_block`, `kernel_vram_dma`. Tests boot. | SP-VRAM-1 |
